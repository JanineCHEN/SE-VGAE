...
from torch.nn import Conv2d as Conv2dLayer
    ...
    def __init__(self,
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if in_channels != 0 and architecture == 'resnet':
            self.skip = Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=True, 
                                    # up=2,
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        self.block_resolutions[-1] = self.img_resolution
    ...
    def forward(self, ws, **block_kwargs):
        ...
        with torch.autograd.profiler.record_function('split_ws'):
            ...
            for res in self.block_resolutions:
                ...
                print("block.num_conv: ",block.num_conv)
                print("block.num_torgb: ",block.num_torgb)
                # https://pytorch.org/docs/stable/generated/torch.narrow.html
                # block_ws.append(ws.narrow(1, w_idx, block.num_conv))
                a = ws.narrow(1, w_idx, block.num_conv + block.num_torgb)
                block_ws.append(a)
                # w_idx += block.num_torgb
                w_idx += block.num_conv
                print("ws.narrow: ",a.shape)
                print("w_idx: ",w_idx)
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.num_ws = self.synthesis.num_ws + 1
...
class DiscriminatorBlock(torch.nn.Module):
    def __init__(self,
        ...
        self.conv0 = Conv2dLayer(tmp_channels, tmp_channels, kernel_size=3, 
                                #  activation=activation,
                                #  trainable=next(trainable_iter), conv_clamp=conv_clamp, channels_last=self.channels_last
                                 )

        self.conv1 = Conv2dLayer(tmp_channels, out_channels, kernel_size=3, 
                                #  activation=activation, down=2,
                                #  trainable=next(trainable_iter), resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=self.channels_last
                                 )

        if architecture == 'resnet':
            self.skip = Conv2dLayer(tmp_channels, out_channels, kernel_size=1, bias=True, 
                                    # down=2,
